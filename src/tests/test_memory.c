...
START_TEST(test_memchr_high_ascii) {  // Символы верхнего ASCII (0x80-0xFF)
  const unsigned char data[] = {0x61, 0x80, 0x63, 0x00};
...
START_TEST(test_memcmp_binary_data) {
  ...
  ck_assert_int_eq(s21_memcmp(d1, d2, 3), memcmp(d1, d2, 3));
}
END_TEST
...
START_TEST(test_memcpy_large_data) {  // Копирование большого объема данных
  ...
  ck_assert_ptr_eq(s21_memcpy(dest_s21, src, buffer_size), dest_s21);
  ck_assert_ptr_eq(memcpy(dest_std, src, buffer_size), dest_std);
  ck_assert_mem_eq(dest_s21, dest_std, sizeof(src));
...
START_TEST(test_memset_zero_length) {
  ...
  ck_assert_ptr_eq(s21_memset(str_s21, 'X', zero_size), str_s21);
  ck_assert_ptr_eq(memset(str_std, 'X', zero_size), str_std);
  ck_assert_str_eq(str_s21, str_std);
}
END_TEST
...
START_TEST(test_memset_null_byte) {
  ...
  ck_assert_ptr_eq(s21_memset(str_s21, '\0', 3), str_s21);
  ck_assert_ptr_eq(memset(str_std, '\0', 3), str_std);
  ck_assert_mem_eq(str_s21, str_std, 6);
}
END_TEST
