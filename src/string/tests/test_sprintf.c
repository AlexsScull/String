...
// Тесты для %c
TEST_SPRINTF(c_minus, "%-5c", 'A', char)
TEST_SPRINTF(c_width, "%5c", 'A', char)
TEST_SPRINTF_2(c_star_width, "%*c", 5, 'A', char)
...
Suite *sprintf_suite_lc_and_ls(void) {
  ...
  tcase_add_test(tc, c_minus);
  tcase_add_test(tc, c_width);
  tcase_add_test(tc, c_star_width);
